...
const UseVoiceChat = () => {
  ...
  useEffect(() => {
    ...
    // iOS specific: Handle visibility change to restart recognition
    const handleVisibilityChange = () => {
      if (!document.hidden && conversationActive && detectIOS()) {
        setTimeout(() => {
          if (!isListening && !speaking && !isProcessing) {
            startRecognition()
          }
        }, 1000)
      }
  ...
  }, [conversationActive, isListening, speaking, isProcessing])
  ...
  useEffect(() => {
    ...
    // iOS needs special handling - continuous mode doesn't work well
    recognitionRef.current.continuous = !isIOS
    recognitionRef.current.interimResults = true
    recognitionRef.current.lang = 'en-US'
    recognitionRef.current.maxAlternatives = 3
    ...
    recognitionRef.current.onresult = (event) => {
      ...
      if (
        ...
        // iOS needs shorter delay for better responsiveness
        silenceTimerRef.current = setTimeout(
          () => {
            if (cleanTranscript.length > 1) {
              processUserMessage(cleanTranscript)
    ...
    recognitionRef.current.onerror = (event) => {
      ...
      // For iOS, we need to be more aggressive about restarting
      if (event.error !== 'aborted' && conversationActive) {
        const restartDelay = isIOS ? 1500 : 1000
        setTimeout(() => {
          if (conversationActive && !isProcessing && !speaking) {
            startRecognition()
          }
    ...
    recognitionRef.current.onend = () => {
      ...
      // iOS requires immediate restart after onend
      if (conversationActive && !isProcessing && !speaking) {
        if (restartTimeoutRef.current) {
          clearTimeout(restartTimeoutRef.current)
        }
  ...
  const startRecognition = () => {
    try {
      if (
        ...
        // iOS requires clearing any existing recognition first
        if (detectIOS() && recognitionRef.current) {
          try {
          ...
          setTimeout(() => {
            try {
              ...
              console.log('iOS recognition start error:', error)
            }
          }, 100)
        } else {
          recognitionRef.current.start()
      ...
      console.error('Start recognition error:', error)
      // For iOS, try again after a delay
      if (detectIOS() && conversationActive) {
        setTimeout(() => {
          if (!isListening && !speaking && !isProcessing) {
            startRecognition()
          }
  ...
  const speakText = (text) => {
    ...
    // iOS requires canceling existing speech first
    if (synthRef.current.speaking) {
      synthRef.current.cancel()
      const delay = detectIOS() ? 800 : 100
    ...
    function startSpeaking() {
      const utterance = new SpeechSynthesisUtterance(text)
      utterance.rate = detectIOS() ? 0.7 : isMobile ? 0.8 : 0.9
      utterance.pitch = 1.0
      utterance.volume = 1.0
      ...
      utterance.onend = () => {
        ...
        if (conversationActive && !isUserSpeaking) {
          const delay = detectIOS() ? 1500 : isMobile ? 1000 : 500
          setTimeout(() => {
            if (
              conversationActive &&
              !isListening &&
  ...
  const toggleConversation = () => {
    if (conversationActive) {
      ...
      setConversationActive(true)
      setIsProcessing(false)
      // iOS needs longer delay to initialize properly
      const delay = detectIOS() ? 2000 : 1000
